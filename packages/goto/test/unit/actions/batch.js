...
test('batchKey marks screenshot groups and treats inject as a barrier', t => {
  t.is(batchKey('screenshot'), 'screenshot')
  t.is(batchKey('inject'), null)
...
test('batchActions keeps consecutive injects ordered', t => {
  ...
  t.is(waves[0].actions[0].type, 'inject')
  t.is(waves[1].actions[0].type, 'inject')
  t.is(waves[2].actions[0].type, 'click')
})
